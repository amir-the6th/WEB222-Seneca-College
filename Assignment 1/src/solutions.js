...
function crush(value) {
  ...
  lower = lower.replace(/\s+/g, '').toLowerCase();
  lower = lower.replace(/-/g, '').toLowerCase();
  lower = lower.replace(/-|\s/g, '').toLowerCase();
...
function createEmailAddress(firstName, lastName, host) {
  ...
  let result;
  if (host) {
    result = `${firstName} ${lastName} <${firstName
      .charAt(0)
      .toLowerCase()}${lastName.toLowerCase()}@${host}>`;
  } else if (firstName.toString().indexOf(' ') >= 0 || lastName.toString().indexOf(' ') >= 0) {
    result = `${firstName} ${lastName} <${firstName.charAt(0).toLowerCase()}${lastName
      .replace(/\s+/g, '')
      .toLowerCase()}@gmail.com>`;
  } else if (firstName.indexOf('-') >= 0 || lastName.toString().indexOf('-') >= 0) {
    result = `${firstName} ${lastName} <${firstName.charAt(0).toLowerCase()}${lastName
      .replace(/-|\s/g, '')
      .toLowerCase()}@gmail.com>`;
  } else {
    result = `${firstName} ${lastName} <${firstName
      .charAt(0)
      .toLowerCase()}${lastName.toLowerCase()}@gmail.com>`;
  }
  return result;
}
...
function ageToRange(age) {
  ...
  let result;
  if (age >= 0 && age <= 19) {
    result = '19 and younger';
  } else if (age >= 20 && age <= 29) {
    result = '20 to 29 Years';
  } else if (age >= 30 && age <= 39) {
    result = '30 to 39 Years';
  } else if (age >= 40 && age <= 49) {
    result = '40 to 49 Years';
  } else if (age >= 50 && age <= 59) {
    result = '50 to 59 Years';
  } else if (age >= 60 && age <= 69) {
    result = '60 to 69 Years';
  } else if (age >= 70 && age <= 79) {
    result = '70 to 79 Years';
  } else if (age >= 80 && age <= 89) {
    result = '80 to 89 Years';
  } else if (age >= 90) {
    result = '90 and older';
...
function toDate(value) {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(value) || !value || value.length === 0) {
    if (!value || value.length === 0) {
      dateNew = new Date();
    } else {
      dateNew = new Date(value.substr(0, 4), value.substr(5, 2) - 1, value.substr(8, 2));
    }
...
// }
// dateNew = new Date(match);
// //dateNew.setFullYear(match[1], match[2], match[3]);

// return dateNew instanceof Date;

// return (
//   dateNew.getFullYear() === match[1] &&
//   dateNew.getMonth() === match[2] - 1 &&
//   dateNew.getDate() === match[3]
// );

//return dateNew.getTime() === dateNew.getTime();
//}
...
function simpleMovingAverage() {
  ...
  //let nums = arguments.filter(n => !isNaN(n)).map(Number);
  //let avg = nums.reduce((x, y) => x + y) / (nums.length || 1);
  ...
  for (var i = 0; i < arguments.length; i++) {
    if (
      ...
      for (i = 0; i < arguments.length; i++) {
        check = Number(arguments[i]);
        sum += check;
      }
      let avg = sum / (arguments.length || 1);
      return avg;
    }
  }
  // if (isNaN(arguments)) {
...
//let avg = arguments.reduce((total, arguments) => total + arguments) / arguments.length;
...
function normalizeBoolean(value) {
  ...
  // if (
  //   lower.toString().indexOf(true) >= 0 ||
  //   lower.toString().indexOf('yes') >= 0 ||
  //   lower.toString().indexOf('1') >= 0
  // ) {
  //   result = true;
  // } else if (
  //   lower.toString().indexOf(false) >= 0 ||
  //   lower.toString().indexOf('no') >= 0 ||
  //   lower.toString().indexOf('-1') >= 0
  // ) {
  //   result = false;
  // }
  ...
  if (/^[Y][E][S]/i.test(value) || /^[Y]/i.test(value)) {
    return true;
  }

  if (/^[T][R][U][E]/i.test(value) || /^[T]/i.test(value)) {
    return true;
  }
  ...
  while (/^[N][O]/i.test(value) || /^[N]/i.test(value)) {
    return false;
  }
  return result;
}
...
function formatQS(id, neighbourhood) {
  ...
  if (!neighbourhood) {
    ...
    result = `?id=${id}&n=${encodeURIComponent(neighbourhood)}`;
  }
  return result;
}
...
function containsID(id, qs) {
  ...
  let idFound = qs.indexOf('id=');
  let neighbourhoodFound = qs.indexOf('&n');

  if (neighbourhoodFound === -1) {
    neighbourhoodFound = qs.length;
  }
  if (Number(qs.substring(idFound + 3, neighbourhoodFound)) === id) {
    result = true;
  } else {
    result = false;
  }
  ...
  //   result = false;
  // } else {
  //   let match = qs.match(id);
  //   result = match && qs === match[0];
  // }
  return result;
}
